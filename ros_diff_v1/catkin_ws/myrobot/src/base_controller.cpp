...
/*****************************************************************************/
float D = 0.14;                       // 轮间距，单位 m
float linear_temp = 9, angular_temp = 9; // 存储线速度和角速度值
float reductionSpeedRatio = 26.666;   // 电机减速比
/*****************************************************************************/

float left_speed = 0, right_speed = 0, left_speed_temp, right_speed_temp;
unsigned char control_flag = 0x07;

// 当前里程计数据：左右轮速度和角度值（编码器）
...
/*****************************************************************************/
void callback(const geometry_msgs::Twist& cmd_input) // cmd_vel话题订阅回调函数
{
    cout << "linear_temp: " << linear_temp << "    " << "angular_temp: " << angular_temp << endl;

    linear_temp = cmd_input.linear.x;  // 读取cmd_vel话题的线速度x分量
    angular_temp = cmd_input.angular.z; // 读取cmd_vel话题的角速度,rad/s
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        // 合成速度合成 取平均速度作为小车当前速度
        p.vx = 0.5 * 0.225 * (right_speed_now + left_speed_now) / 60;

        // 目前选用角度变化量除以时间计算角速度，也可以根据速度计算单轮差速
